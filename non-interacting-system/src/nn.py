...
class NN_new(torch.jit.ScriptModule):
    def __init__(self, input_dim, hid_dim, output_dim, h:float, m:float, om:float, x0_phase:float, w1:float, w2:float):
        super(NN_new, self).__init__()
        self.h = h
        self.m = m
        self.om = om
        self.x0_phase = x0_phase
        self.w1 = w1
        self.w2 = w2
    ...
    def forward(self, x, t):
        ...
        v0_val = v_0(x, h=self.h, m=self.m, s0_coeff=self.x0_phase)
...
class NN_non_interact(torch.jit.ScriptModule):
    def __init__(self, input_dim, hid_dim, output_dim, h:float, m:float, om:float, sig2:float, x0_phase:float, w1:float, w2:float):
        super(NN_non_interact, self).__init__()
        self.h = h
        self.m = m
        self.om = om
        self.sig2 = sig2
    ...
    @torch.jit.script_method
    def forward(self, x, t):
        u0_val = u_0(x, om=self.om, h=self.h, m=self.m, sig2=self.sig2)
        v0_val = v_0(x, h=self.h, m=self.m, s0_coeff=self.x0_phase)
        out = torch.hstack((x, t))
        out = self.relu1(self.fc1(out))
        out = self.relu2(self.fc2(out))
...
def train_dsm_non_interact(net_u, net_v,
                       device,
                       optimizer,
                       criterion,
                       alpha, beta, gamma,
